...
const float fpsValues[] = {60.0, 48.0, 30.0, 24.0, 20.0, 18.0, 15.0, 12.0, 10.0, 9.0, 8.0, 6.0, 5.0, 4.0, 3.0, 2.0, 1.5}; // Frames per second
const int numFPS = 17;
int fpsIndex = 12; // Default to 5 FPS
...
// Shutter speed array in microseconds: .1 .15 .2 .3 .4 .5 1 2 3 4 5 8 10 16 25 32 50 60 120 180
...
const int numSpeeds = 20;
int speedIndex = 5; // Default to .5s (index 5)
...
void runBothCycle() {
  // Run shutter and claw simultaneously with precise timing (no jitter)
  // Uses event-based approach with blocking delays instead of polling loop
  // Includes acceleration ramping for high-speed operation
  ...
  // Shutter: 200 steps over full cycle time with ramping
  int shutterSteps = 200;
  
  // Claw: 35 steps, reliable speed to fit in ~18.5ms with ramping
  int clawSteps = 35;
  long targetClawSpeed = 450; // Target: 450μs per step (more reliable!)
  long clawTotalMicros = clawSteps * targetClawSpeed; // ~15.75ms at constant speed
  long clawPulseMicros = clawTotalMicros / (clawSteps * 2); // Time per pulse (HIGH or LOW)
  
  // Calculate shutter ramping parameters
  // Single ramp-up at start: aim for ~200ms ramp time
  long minStepMicros = 3000; // Start slow: 3000μs per step
  long targetStepMicros = cycleTimeMicros / shutterSteps; // Target speed
  
  // Calculate ramp steps to reach ~200ms
  int rampSteps;
  if (targetStepMicros >= minStepMicros) {
    rampSteps = 0; // No ramping needed if already slow
  } else {
    // Ramp over ~200ms
    long avgStepTime = (minStepMicros + targetStepMicros) / 2;
    rampSteps = 200000 / avgStepTime; // ~200ms worth of steps
    if (rampSteps > shutterSteps / 2) rampSteps = shutterSteps / 2; // Cap at 50%
  }
  
  // Enable both motors and set direction
  digitalWrite(en1, HIGH);
  digitalWrite(en2, HIGH);
  digitalWrite(dir1, HIGH);
  digitalWrite(dir2, HIGH);
  
  // SHUTTER: Single ramp-up at start
  long constantPulseDelay = targetStepMicros / 2;
  
  // Initial ramp-up for shutter
  for (int step = 0; step < rampSteps; step++) {
    float progress = (float)step / rampSteps;
    long stepDelay = minStepMicros - (long)((minStepMicros - targetStepMicros) * progress);
    long pulseDelay = stepDelay / 2;
  ...
  // CLAW ramping settings - 30% ramp, 1000μs to 450μs
  int clawRampSteps = (clawSteps * 3) / 10; // 30% ramp = 10 steps
  long minClawStepMicros = 1000; // Start at 1000μs
  long targetClawStepMicros = targetClawSpeed; // End at 450μs (reliable speed!)
  
  if (targetClawStepMicros >= minClawStepMicros) {
    minClawStepMicros = targetClawStepMicros;
    clawRampSteps = 0;
  }
  
  // Pre-calculate claw step timings with ramping
  long clawStepDelays[35]; // Max 35 steps
  for (int step = 0; step < clawSteps; step++) {
    if (step < clawRampSteps) {
      float progress = (float)step / clawRampSteps;
      clawStepDelays[step] = minClawStepMicros - (long)((minClawStepMicros - targetClawStepMicros) * progress);
    } else {
      clawStepDelays[step] = targetClawStepMicros;
    }
  }
  
  // NOW RUN FILMING CYCLES - SIMULTANEOUS EXECUTION
  for (int cycle = 0; cycle < numCycles; cycle++) {
    // Track both motors
    int shutterStepsThisCycle = shutterSteps;
    if (cycle == 0) shutterStepsThisCycle = shutterSteps - rampSteps;
    ...
    unsigned long nextShutterMicros = 0;
    unsigned long nextClawMicros = 0;
    unsigned long currentTime = 0;
    
    // Run both motors simultaneously
    while (shutterStep < shutterStepsThisCycle || clawStep < clawSteps) {
      // Calculate next event times
      unsigned long nextShutterEvent = (shutterStep < shutterStepsThisCycle) ? nextShutterMicros : 4294967295UL;
      unsigned long nextClawEvent = (clawStep < clawSteps) ? nextClawMicros : 4294967295UL;
      
      // Find earliest event
      unsigned long nextEvent = (nextShutterEvent <= nextClawEvent) ? nextShutterEvent : nextClawEvent;
      bool doShutter = (nextShutterEvent <= nextClawEvent) && (shutterStep < shutterStepsThisCycle);
      bool doClaw = (nextClawEvent <= nextShutterEvent) && (clawStep < clawSteps);
      
      // Wait precisely until next event
      unsigned long waitTime = nextEvent - currentTime;
      if (waitTime > 0) {
        if (waitTime > 16000) {
          delay(waitTime / 1000);
      ...
      // Execute event(s)
      if (doShutter) {
        digitalWrite(step1, shutterHigh ? LOW : HIGH);
        if (shutterHigh) {
          shutterStep++;
          shutterHigh = false;
        } else {
        ...
        nextShutterMicros = currentTime + constantPulseDelay;
      }
      
      if (doClaw) {
        digitalWrite(step2, clawHigh ? LOW : HIGH);
        if (clawHigh) {
          clawStep++;
          clawHigh = false;
        } else {
        ...
        long clawPulseDelay = clawStepDelays[clawStep < clawSteps ? clawStep : clawSteps - 1] / 2;
        nextClawMicros = currentTime + clawPulseDelay;
      }
    }
  }
